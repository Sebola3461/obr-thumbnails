...
export class Palette {
	...
	private static async getTopColors(
		image: Types.InputImage,
		colorCount: number
	): Promise<Types.palette> {
		...
		if (beginIndex === -1 || endIndex === -1)
			throw new Error(
				`PALETTE_DETECTION_FAILED: Image not found or graphicsmagik not installed.`
			);
